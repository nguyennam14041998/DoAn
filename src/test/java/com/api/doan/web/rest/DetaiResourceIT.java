...
public class DetaiResourceIT {
    ...
    @Test
    @Transactional
    public void getAllDetaisByCoquanphoihopIsEqualToSomething() throws Exception {
        // Initialize the database
        detaiRepository.saveAndFlush(detai);
        Coquanphoihop coquanphoihop = CoquanphoihopResourceIT.createEntity(em);
        em.persist(coquanphoihop);
        em.flush();
        detai.addCoquanphoihop(coquanphoihop);
        detaiRepository.saveAndFlush(detai);
        Long coquanphoihopId = coquanphoihop.getId();

        // Get all the detaiList where coquanphoihop equals to coquanphoihopId
        defaultDetaiShouldBeFound("coquanphoihopId.equals=" + coquanphoihopId);

        // Get all the detaiList where coquanphoihop equals to coquanphoihopId + 1
        defaultDetaiShouldNotBeFound("coquanphoihopId.equals=" + (coquanphoihopId + 1));
    }
