...
public class CoquanphoihopResourceIT {
    ...
    @Mock
    private CoquanphoihopRepository coquanphoihopRepositoryMock;

    @Autowired
    private CoquanphoihopMapper coquanphoihopMapper;

    @Mock
    private CoquanphoihopService coquanphoihopServiceMock;
    ...
    public void getAllCoquanphoihopsWithEagerRelationshipsIsEnabled() throws Exception {
        CoquanphoihopResource coquanphoihopResource = new CoquanphoihopResource(coquanphoihopServiceMock, coquanphoihopQueryService);
        when(coquanphoihopServiceMock.findAllWithEagerRelationships(any())).thenReturn(new PageImpl(new ArrayList<>()));

        MockMvc restCoquanphoihopMockMvc = MockMvcBuilders.standaloneSetup(coquanphoihopResource)
            .setCustomArgumentResolvers(pageableArgumentResolver)
            .setControllerAdvice(exceptionTranslator)
            .setConversionService(createFormattingConversionService())
            .setMessageConverters(jacksonMessageConverter).build();

        restCoquanphoihopMockMvc.perform(get("/api/coquanphoihops?eagerload=true"))
        .andExpect(status().isOk());

        verify(coquanphoihopServiceMock, times(1)).findAllWithEagerRelationships(any());
    }

    @SuppressWarnings({"unchecked"})
    public void getAllCoquanphoihopsWithEagerRelationshipsIsNotEnabled() throws Exception {
        CoquanphoihopResource coquanphoihopResource = new CoquanphoihopResource(coquanphoihopServiceMock, coquanphoihopQueryService);
            when(coquanphoihopServiceMock.findAllWithEagerRelationships(any())).thenReturn(new PageImpl(new ArrayList<>()));
            MockMvc restCoquanphoihopMockMvc = MockMvcBuilders.standaloneSetup(coquanphoihopResource)
            .setCustomArgumentResolvers(pageableArgumentResolver)
    ...
    @Test
    @Transactional
    public void getAllCoquanphoihopsByDetaiIsEqualToSomething() throws Exception {
        // Initialize the database
        coquanphoihopRepository.saveAndFlush(coquanphoihop);
        Detai detai = DetaiResourceIT.createEntity(em);
        em.persist(detai);
        em.flush();
        coquanphoihop.addDetai(detai);
        coquanphoihopRepository.saveAndFlush(coquanphoihop);
        Long detaiId = detai.getId();

        // Get all the coquanphoihopList where detai equals to detaiId
        defaultCoquanphoihopShouldBeFound("detaiId.equals=" + detaiId);

        // Get all the coquanphoihopList where detai equals to detaiId + 1
        defaultCoquanphoihopShouldNotBeFound("detaiId.equals=" + (detaiId + 1));
    }
