...
public class NhansuResourceIT {
    ...
    @Mock
    private NhansuRepository nhansuRepositoryMock;

    @Autowired
    private NhansuMapper nhansuMapper;

    @Mock
    private NhansuService nhansuServiceMock;
    ...
    public void getAllNhansusWithEagerRelationshipsIsEnabled() throws Exception {
        NhansuResource nhansuResource = new NhansuResource(nhansuServiceMock, nhansuQueryService);
        when(nhansuServiceMock.findAllWithEagerRelationships(any())).thenReturn(new PageImpl(new ArrayList<>()));

        MockMvc restNhansuMockMvc = MockMvcBuilders.standaloneSetup(nhansuResource)
            .setCustomArgumentResolvers(pageableArgumentResolver)
            .setControllerAdvice(exceptionTranslator)
            .setConversionService(createFormattingConversionService())
            .setMessageConverters(jacksonMessageConverter).build();

        restNhansuMockMvc.perform(get("/api/nhansus?eagerload=true"))
        .andExpect(status().isOk());

        verify(nhansuServiceMock, times(1)).findAllWithEagerRelationships(any());
    }

    @SuppressWarnings({"unchecked"})
    public void getAllNhansusWithEagerRelationshipsIsNotEnabled() throws Exception {
        NhansuResource nhansuResource = new NhansuResource(nhansuServiceMock, nhansuQueryService);
            when(nhansuServiceMock.findAllWithEagerRelationships(any())).thenReturn(new PageImpl(new ArrayList<>()));
            MockMvc restNhansuMockMvc = MockMvcBuilders.standaloneSetup(nhansuResource)
            .setCustomArgumentResolvers(pageableArgumentResolver)
