...
public class NguonkinhphiResourceIT {
    ...
    @Mock
    private NguonkinhphiRepository nguonkinhphiRepositoryMock;

    @Autowired
    private NguonkinhphiMapper nguonkinhphiMapper;

    @Mock
    private NguonkinhphiService nguonkinhphiServiceMock;
    ...
    public void getAllNguonkinhphisWithEagerRelationshipsIsEnabled() throws Exception {
        NguonkinhphiResource nguonkinhphiResource = new NguonkinhphiResource(nguonkinhphiServiceMock, nguonkinhphiQueryService);
        when(nguonkinhphiServiceMock.findAllWithEagerRelationships(any())).thenReturn(new PageImpl(new ArrayList<>()));

        MockMvc restNguonkinhphiMockMvc = MockMvcBuilders.standaloneSetup(nguonkinhphiResource)
            .setCustomArgumentResolvers(pageableArgumentResolver)
            .setControllerAdvice(exceptionTranslator)
            .setConversionService(createFormattingConversionService())
            .setMessageConverters(jacksonMessageConverter).build();

        restNguonkinhphiMockMvc.perform(get("/api/nguonkinhphis?eagerload=true"))
        .andExpect(status().isOk());

        verify(nguonkinhphiServiceMock, times(1)).findAllWithEagerRelationships(any());
    }

    @SuppressWarnings({"unchecked"})
    public void getAllNguonkinhphisWithEagerRelationshipsIsNotEnabled() throws Exception {
        NguonkinhphiResource nguonkinhphiResource = new NguonkinhphiResource(nguonkinhphiServiceMock, nguonkinhphiQueryService);
            when(nguonkinhphiServiceMock.findAllWithEagerRelationships(any())).thenReturn(new PageImpl(new ArrayList<>()));
            MockMvc restNguonkinhphiMockMvc = MockMvcBuilders.standaloneSetup(nguonkinhphiResource)
            .setCustomArgumentResolvers(pageableArgumentResolver)
