...
export const getEntities: ICrudGetAllAction<IHoidongdanhgia> = (page, size, sort) => {
  const requestUrl = `${apiUrl}${sort ? `?page=${page}&size=${size}&sort=${sort}` : ''}`;
  return {
    type: ACTION_TYPES.FETCH_HOIDONGDANHGIA_LIST,
    payload: axios.get<IHoidongdanhgia>(`${requestUrl}${sort ? '&' : '?'}cacheBuster=${new Date().getTime()}`)
  };
};
